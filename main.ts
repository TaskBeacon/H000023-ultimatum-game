...
export async function run(root: HTMLElement): Promise<void> {
  ...
  await mountTaskApp({
    root,
    task_id: "H000023-ultimatum-game",
    task_name: "Ultimatum Game",
    task_description: "HTML preview aligned to local psyflow Ultimatum Game procedure and parameters.",
    settings,
    subInfo,
    stimBank,
    buildTrials: (): CompiledTrial[] => {
      ...
      for (let blockIndex = 0; blockIndex < totalBlocks; blockIndex += 1) {
        ...
        const plannedConditions = controller.prepare_block({
          ...
          conditions: (Array.isArray(settings.conditions) ? settings.conditions : ["fair", "unfair", "very_unfair"]).map(
            String
          )
        });
